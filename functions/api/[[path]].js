...
async function isBlocked(db, a, b) { return db.prepare(`SELECT 1 FROM blocks WHERE (blocker_id=? AND blocked_id=?) OR (blocker_id=? AND blocked_id=?)`).bind(a,b,b,a).first(); }
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/me' && method === 'GET') { const [w,v]=await Promise.all([env.DB.prepare('SELECT balance FROM wallets WHERE user_id=?').bind(user.id).first(),env.DB.prepare('SELECT status FROM verification_requests WHERE user_id=? ORDER BY id DESC LIMIT 1').bind(user.id).first()]); return json({ok:true,user:{...user,password_hash:undefined,password_salt:undefined,balance:w?.balance||0,verification_status:v?.status||user.adult_status||'unverified'}}); }
    if (path === '/me' && method === 'PATCH') { const d=await body(request); await env.DB.prepare(`UPDATE users SET name=?,bio=?,gender=?,looking_for=?,approximate_area=?,call_price=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(clean(d.name||user.name,30),clean(d.bio,500),clean(d.gender||user.gender,30),clean(d.lookingFor||user.looking_for,30),clean(d.area,80),Math.max(5,Math.min(500,Number(d.callPrice||user.call_price))),user.id).run();await env.DB.prepare(`UPDATE users SET paid_call_eligible=CASE WHEN adult_status='verified' AND photo_data IS NOT NULL AND LENGTH(TRIM(bio))>=20 THEN 1 ELSE 0 END WHERE id=?`).bind(user.id).run(); return json({ok:true}); }
    if (path === '/me/photo' && method === 'POST') { const d=await body(request), photo=String(d.photo||''); if(!/^data:image\/(jpeg|png|webp);base64,/.test(photo)||photo.length>2800000)return fail('Use a JPEG, PNG or WebP under 2 MB.'); await env.DB.prepare('UPDATE users SET photo_data=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(photo,user.id).run();await env.DB.prepare(`UPDATE users SET paid_call_eligible=CASE WHEN adult_status='verified' AND photo_data IS NOT NULL AND LENGTH(TRIM(bio))>=20 THEN 1 ELSE 0 END WHERE id=?`).bind(user.id).run(); return json({ok:true}); }
    if (path === '/profiles' && method === 'GET') { const community=await env.DB.prepare(`SELECT COUNT(*) AS total FROM users WHERE status='active' AND is_demo=0`).first(),threshold=Math.max(1,Number(env.REAL_DISCOVERY_MIN_USERS||100)),showReal=Number(community?.total||0)>=threshold;const rows=await env.DB.prepare(`SELECT id,name,CAST((julianday('now')-julianday(dob))/365.2425 AS INTEGER) age,bio,approximate_area,photo_data,is_ai,is_demo,adult_status,call_price FROM users WHERE id<>? AND status='active' AND (?=1 OR is_demo=1) AND id NOT IN (SELECT blocked_id FROM blocks WHERE blocker_id=?) AND id NOT IN (SELECT blocker_id FROM blocks WHERE blocked_id=?) ORDER BY is_demo DESC,created_at DESC LIMIT 50`).bind(user.id,showReal?1:0,user.id,user.id).all(); return json({ok:true,profiles:rows.results,discoveryMode:showReal?'community':'ai_demo',realUsers:Number(community?.total||0),realDiscoveryThreshold:threshold}); }
    ...
    if (path==='/matches'&&method==='GET') { const rows=await env.DB.prepare(`SELECT m.id,u.id user_id,u.name,u.photo_data,u.is_ai,u.call_price,m.created_at,(SELECT msg.id FROM messages msg WHERE msg.match_id=m.id ORDER BY msg.id DESC LIMIT 1) latest_message_id,(SELECT msg.body FROM messages msg WHERE msg.match_id=m.id ORDER BY msg.id DESC LIMIT 1) latest_message,(SELECT msg.sender_id FROM messages msg WHERE msg.match_id=m.id ORDER BY msg.id DESC LIMIT 1) latest_sender_id,(SELECT msg.created_at FROM messages msg WHERE msg.match_id=m.id ORDER BY msg.id DESC LIMIT 1) latest_message_at FROM matches m JOIN users u ON u.id=CASE WHEN m.user_a=? THEN m.user_b ELSE m.user_a END WHERE (m.user_a=? OR m.user_b=?) AND u.status='active' AND u.id NOT IN (SELECT blocked_id FROM blocks WHERE blocker_id=?) AND u.id NOT IN (SELECT blocker_id FROM blocks WHERE blocked_id=?) ORDER BY COALESCE(latest_message_at,m.created_at) DESC,m.id DESC`).bind(user.id,user.id,user.id,user.id,user.id).all(); return json({ok:true,matches:rows.results}); }
    if (/^\/matches\/\d+\/messages$/.test(path)) { const matchId=Number(path.split('/')[2]), match=await env.DB.prepare('SELECT * FROM matches WHERE id=? AND (user_a=? OR user_b=?)').bind(matchId,user.id,user.id).first(); if(!match)return fail('Match not found.',404);const otherId=match.user_a===user.id?match.user_b:match.user_a;if(await isBlocked(env.DB,user.id,otherId))return fail('This conversation is blocked.',403); if(method==='GET'){const rows=await env.DB.prepare('SELECT id,sender_id,body,kind,gift_id,created_at FROM messages WHERE match_id=? ORDER BY id ASC LIMIT 200').bind(matchId).all();return json({ok:true,messages:rows.results});} if(method==='POST'){const d=await body(request),text=clean(d.body,1000);if(!text)return fail('Message is empty.');const r=await env.DB.prepare('INSERT INTO messages(match_id,sender_id,body) VALUES(?,?,?)').bind(matchId,user.id,text).run(),other=await env.DB.prepare('SELECT id,name,is_ai,is_demo FROM users WHERE id=?').bind(otherId).first();let aiReply=false;if(other?.is_ai&&other?.is_demo){const replies=[`Hi ${user.name}! I’m a clearly labelled AI demo profile. Thanks for testing NearVibe 👋`,`That sounds good. I’m an AI demo, so please don’t share personal or financial information with me.`,`Thanks for the message! This reply is generated by NearVibe’s safe demo system.`],reply=replies[r.meta.last_row_id%replies.length];await env.DB.prepare(`INSERT INTO messages(match_id,sender_id,body,kind) VALUES(?,?,?,'ai_demo')`).bind(matchId,otherId,reply).run();aiReply=true;}return json({ok:true,id:r.meta.last_row_id,aiReply},201);} }
    ...
    if(/^\/matches\/\d+\/gifts$/.test(path)&&method==='POST'){const matchId=Number(path.split('/')[2]),m=await env.DB.prepare('SELECT * FROM matches WHERE id=? AND (user_a=? OR user_b=?)').bind(matchId,user.id,user.id).first();if(!m)return fail('Match not found.',404);const recipient=m.user_a===user.id?m.user_b:m.user_a;if(await isBlocked(env.DB,user.id,recipient))return fail('This conversation is blocked.',403);const d=await body(request),gift=await env.DB.prepare('SELECT * FROM gifts WHERE id=? AND active=1').bind(clean(d.giftId,30)).first();if(!gift)return fail('Gift not found.',404);const idem=`gift:${user.id}:${crypto.randomUUID()}`;const bal=await transfer(env.DB,user.id,-gift.coin_cost,'gift','match',String(matchId),idem,JSON.stringify({giftId:gift.id,recipient}));await env.DB.prepare(`INSERT INTO messages(match_id,sender_id,body,kind,gift_id) VALUES(?,?,?,?,?)`).bind(matchId,user.id,`${gift.emoji} ${gift.name}`,'gift',gift.id).run();return json({ok:true,balance:bal});}
    if(/^\/matches\/\d+\/call-offers$/.test(path)&&method==='POST'){const matchId=Number(path.split('/')[2]),m=await env.DB.prepare('SELECT * FROM matches WHERE id=? AND (user_a=? OR user_b=?)').bind(matchId,user.id,user.id).first();if(!m)return fail('Match not found.',404);const recipient=m.user_a===user.id?m.user_b:m.user_a;if(await isBlocked(env.DB,user.id,recipient))return fail('This conversation is blocked.',403);const d=await body(request),rate=Math.max(5,Math.min(500,Number(d.rate)));if(!Number.isFinite(rate))return fail('Choose a valid call rate.');const recipientUser=await env.DB.prepare('SELECT is_ai,is_demo,paid_call_eligible FROM users WHERE id=?').bind(recipient).first();if(!recipientUser)return fail('Account not found.',404);if(!(recipientUser.is_ai&&recipientUser.is_demo)&&!recipientUser.paid_call_eligible)return fail('This member is not yet eligible for paid calls.',403);const r=await env.DB.prepare('INSERT INTO call_offers(match_id,proposer_id,recipient_id,coins_per_minute,parent_offer_id) VALUES(?,?,?,?,?)').bind(matchId,user.id,recipient,rate,d.parentOfferId||null).run();if(recipientUser.is_ai&&recipientUser.is_demo){await env.DB.prepare(`UPDATE call_offers SET status='accepted',responded_at=CURRENT_TIMESTAMP WHERE id=?`).bind(r.meta.last_row_id).run();const call=await env.DB.prepare('INSERT INTO calls(match_id,payer_id,recipient_id,rate) VALUES(?,?,?,?)').bind(matchId,user.id,recipient,rate).run();return json({ok:true,id:r.meta.last_row_id,rate,accepted:true,callId:call.meta.last_row_id},201);}return json({ok:true,id:r.meta.last_row_id,rate,accepted:false},201);}
    ...
    if(/^\/calls\/\d+\/(tick|end)$/.test(path)&&method==='POST'){const [, ,id,action]=path.split('/'),call=await env.DB.prepare(`SELECT *,CAST((julianday('now')-julianday(last_billed_at))*86400 AS INTEGER) elapsed_seconds FROM calls WHERE id=? AND (payer_id=? OR recipient_id=?) AND status='active'`).bind(Number(id),user.id,user.id).first();if(!call)return fail('Active call not found.',404);if(action==='end'){await env.DB.prepare(`UPDATE calls SET status='ended',ended_at=CURRENT_TIMESTAMP WHERE id=?`).bind(call.id).run();return json({ok:true});}if(user.id!==call.payer_id)return fail('Only the payer may bill this call.',403);if(call.billed_minutes>0&&Number(call.elapsed_seconds)<55)return fail('The next call minute is not due yet.',429);const minute=call.billed_minutes+1,idem=`call:${call.id}:minute:${minute}`,bal=await transfer(env.DB,user.id,-call.rate,'call_minute','call',String(call.id),idem,JSON.stringify({minute}));await env.DB.prepare(`UPDATE calls SET billed_minutes=?,last_billed_at=CURRENT_TIMESTAMP WHERE id=?`).bind(minute,call.id).run();return json({ok:true,balance:bal,billedMinutes:minute});}
    if(path==='/me/blocks'&&method==='GET'){const r=await env.DB.prepare(`SELECT u.id,u.name,u.photo_data,b.created_at FROM blocks b JOIN users u ON u.id=b.blocked_id WHERE b.blocker_id=? ORDER BY b.created_at DESC`).bind(user.id).all();return json({ok:true,blocks:r.results});}
    if(/^\/users\/\d+\/block$/.test(path)&&method==='DELETE'){const target=Number(path.split('/')[2]);await env.DB.prepare('DELETE FROM blocks WHERE blocker_id=? AND blocked_id=?').bind(user.id,target).run();return json({ok:true});}
    if(/^\/users\/\d+\/(report|block)$/.test(path)&&method==='POST'){const [, ,id,action]=path.split('/'),target=Number(id);if(target===user.id)return fail('You cannot report or block yourself.');const targetUser=await env.DB.prepare('SELECT id FROM users WHERE id=?').bind(target).first();if(!targetUser)return fail('Account not found.',404);if(action==='block'){await env.DB.prepare('INSERT OR IGNORE INTO blocks(blocker_id,blocked_id) VALUES(?,?)').bind(user.id,target).run();return json({ok:true});}const d=await body(request);await env.DB.prepare('INSERT INTO reports(reporter_id,reported_user_id,reason,details) VALUES(?,?,?,?)').bind(user.id,target,clean(d.reason,100),clean(d.details,1000)).run();return json({ok:true},201);}
    if(path==='/verification/request'&&method==='POST'){if(user.adult_status==='verified')return json({ok:true,status:'approved'});const existing=await env.DB.prepare(`SELECT status FROM verification_requests WHERE user_id=? ORDER BY id DESC LIMIT 1`).bind(user.id).first();if(existing?.status==='pending')return json({ok:true,status:'pending'});await env.DB.prepare(`INSERT INTO verification_requests(user_id,status) VALUES(?,'pending')`).bind(user.id).run();return json({ok:true,status:'pending'},201);}
    ...
    if(path.startsWith('/admin/')) {
      ...
      if(/^\/admin\/verifications\/\d+\/(approve|reject)$/.test(path)&&method==='POST'){
        ...
        await env.DB.batch([env.DB.prepare(`UPDATE verification_requests SET status=?,reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=?`).bind(status,user.id,requestId),env.DB.prepare(`UPDATE users SET adult_status=?,paid_call_eligible=CASE WHEN ?='verified' AND photo_data IS NOT NULL AND LENGTH(TRIM(bio))>=20 THEN 1 ELSE 0 END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(adultStatus,adultStatus,verification.user_id),env.DB.prepare(`INSERT INTO audit_log(actor_id,action,target_type,target_id) VALUES(?,?, 'verification',?)`).bind(user.id,`verification_${status}`,String(requestId))]);
